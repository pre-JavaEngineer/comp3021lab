	...
	*/
	...
	private HBox addHBox() {
		...
		buttonSearch.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event){
				...
				if(currentFolder != "-----")
				{
					...
					searchedNotes.addAll(noteBook.getFolderByName(currentFolder).searchNotes(currentSearch));
		...
		buttonRemove.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event){
				ArrayList<String> list = new ArrayList<String>();
				for(Note n:noteBook.getFolderByName(currentFolder).getNotes())
				{
					list.add(n.getTitle());
				}
				ObservableList<String> combox2 = FXCollections.observableArrayList(list);
	...
	private VBox addVBox() {
		...
		titleslistView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Object>() {
			...
			public void changed(ObservableValue ov, Object t, Object t1) {
				...
				// textAreNote
				String content = "";
				content = noteBook.getFolderByName(currentFolder).getNoteByTitle(currentNote).getContent();
		...
		buttonAddFolder.setOnAction(new EventHandler<ActionEvent>()
				{
					public void handle(ActionEvent event)
					{
						...
						if (result.isPresent())
						{
							...
							else if(noteBook.getFolderByName(stringResult) != null)
							{
								Alert alert = new Alert(AlertType.WARNING);
								alert.setTitle("Warning");
								alert.setContentText("You already have a folder named with " + stringResult);
								alert.showAndWait();
							}
							else
							{
								noteBook.addFolder(stringResult);
								foldersComboBox.getItems().add(stringResult);
								currentFolder = stringResult;
								foldersComboBox.getSelectionModel().select(stringResult);
							}
						}
					}
				});
		...
		buttonAddNote.setOnAction(new EventHandler<ActionEvent>()
		{
			public void handle(ActionEvent event)
			{
				if (noteBook.getFolderByName(currentFolder) == null)
				{
					Alert alert = new Alert(AlertType.WARNING);
					alert.setTitle("Warning");
				...
				else
				{
					...
					{
						String stringResult = result.get();
						if (noteBook.createTextNote(currentFolder, stringResult))
						{
							Alert alert = new Alert(AlertType.INFORMATION);
							alert.setTitle("Successful!");
							alert.setContentText("Insert " + stringResult + " to folder " + currentFolder + " successfully!");
							alert.showAndWait();
							updateListView();
						}
					}
	...
	private void updateListView() {
		...
		// currentFolder
		if(currentFolder != "-----" && noteBook.getFolderByName(currentFolder) != null)
		{
			for(Note note:noteBook.getFolderByName(currentFolder).getNotes())
			{
	...
	private GridPane addGridPane() {
		...
		buttonSaveNote.setOnAction(new EventHandler<ActionEvent>()
		{
			public void handle(ActionEvent event)
			{
				if (noteBook.getFolderByName(currentFolder) == null || noteBook.getFolderByName(currentFolder).getNoteByTitle(currentNote) == null )
				{
					Alert alert = new Alert(AlertType.WARNING);
					alert.setTitle("Warning");
				...
				{
					TextNote selectedNote = (TextNote) noteBook.getFolderByName(currentFolder).getNoteByTitle(currentNote);
					selectedNote.setContent(textAreaNote.getText());
				}
			}
		});
		...
		buttonDeleteNote.setOnAction(new EventHandler<ActionEvent>()
		{
			public void handle(ActionEvent event)
			{
				...
				{
					if(noteBook.getFolderByName(currentFolder).removeNotes(currentNote))
					{
